...
prompt = f"Your task is to optimize the provided code digitrecGen.c by adding HLS pragmas or directives that minimize latency by roughly one order of magnitude while maintaining the original functionality. You can apply loop unrolling to different functions for parallelism and array partitioning to split arrays into smaller arrays. Use only fixed-bound loops, and ensure the baseline code structure remains unchanged apart from the added pragmas or directives. The final design should preserve functionality while achieving a significant reduction in latency. Here is the digitrecGen.cpp code:\n\n{cpp_code}"
# Initialize variables for tracking attempts
attempts = 1
